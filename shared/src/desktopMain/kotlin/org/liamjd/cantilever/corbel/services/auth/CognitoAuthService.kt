...
class CognitoAuthService : AuthenticationService {
    ...
    val server = embeddedServer(Netty, port = 44817, configure = {
        ...
        routing {
            get("/callback") {
                ...
                call.respondText("Hello from embedded server in Corbel")
            }
        }
    }.start(wait = false)
    ...
    private val localCallbackUrl = URLEncoder.encode("https://127.0.0.1:44817/callback", Charset.defaultCharset())
    ...
    override suspend fun login(user: SubmitUser) {
...
//        https://cantilever.auth.eu-west-2.amazoncognito.com/login?client_id=128mn8agtcn7d7giqn0gk9rhni&response_type=code&scope=aws.cognito.signin.user.admin+email+openid&redirect_uri=corbelApp%3A%2F%2Fauth
        ...
        val cognitoUrl = "https://cantilever.auth.eu-west-2.amazoncognito.com/oauth2/authorize?response_type=code&client_id=$clientAppId&redirect_uri=$localCallbackUrl&scope=aws.cognito.signin.user.admin+email+openid"
        try {
